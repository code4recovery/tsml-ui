...
export default function LoadData(meetings, capabilities) {
	...
	for (let i = 0; i < meetings.length; i++) {
		...
		if (meeting.time) {
			...
			let times = [];
			if (timeInMinutes >= 240 && timeInMinutes < 720) { //4am–12pm
				times.push(0); //morning
			}
			if (timeInMinutes >= 660 && timeInMinutes < 1020) { //11am–5pm
				times.push(1); //midday
			}
			if (timeInMinutes >= 960 && timeInMinutes < 1260) { //4–9pm
				times.push(2); //evening
			}
			if (timeInMinutes >= 1200 || timeInMinutes < 300) { //8pm–5am
				times.push(3); //night
			}
		...
		//build search string
		meeting.search = [meeting.name, meeting.location, meeting.location_notes, meeting.notes, meeting.formatted_address].filter(e => e).join(' ').toLowerCase();
